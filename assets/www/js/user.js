...
function injectUserFuncs(app) {
	...
	if (userInfo === null) {
		...
		loginDialog.noCancelOnEscKey = true;
		loginDialog.noCancelOnOutsideClick = true;
		loginDialog.open();
	} else {
		logined();
	}
	...
	startChat = function(inviteUserId, targetUserId, opponentUserName) {
		...
		onChat = chatRef.on('child_added', function(snap) {
			...
			if (info.userId !== userInfo.uid) {
				...
				if (app.youtubePlayer.isPlaying === true && app.youtubePlayer.pauseVideo !== undefined && isConnected !== true) {
				...
				// 동영상을 타임라인을 이동했다고 전송받음
				if (info.isMoved === true) {
					if (app.youtubePlayer.seekTo !== undefined) {
						// 동영상 타임라인 이동
						app.youtubePlayer.seekTo(info.currentTime);
						app.youtubePlayer.lastTime = info.currentTime;
